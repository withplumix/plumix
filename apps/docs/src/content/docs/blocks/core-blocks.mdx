...
Seventeen of the 18 declare a category, and the editor's inserter groups its palette by it. Four appear here, `text`, `layout`, `media` and `interactive`. `core/pattern-ref` declares none, and the inserter files a category-less spec under `uncategorized`. Five of the 18 set `inserter: false` and never appear in the palette at all, because something else creates them. `core/table` seeds the four table row and cell blocks, and inserting a pattern that registers itself as a reference writes a `core/pattern-ref`.
