...
The `core/` namespace is closed. A plugin or a theme cannot register a name inside it, which is why these 18 are always present rather than something a site opts into. They can still be replaced. Boot merges the registry core first, then plugins, then the theme, and a later layer wins a name collision.

Each block declares a category, and the editor's inserter groups its palette by it. Four categories appear here: `text`, `layout`, `media` and `interactive`. Five of the 18 set `inserter: false` and never appear in the palette at all, because something else creates them. `core/table` seeds the four table row and cell blocks, and inserting a pattern that registers itself as a reference writes a `core/pattern-ref`.

The examples below are stored nodes, the shape a block takes inside an entry's content tree. Each one is a value you could put in the `blocks` array of an entry's content. [Overview](/blocks/overview/) covers what those fields mean and how the renderer walks the tree.

## The core blocks

### `core/rich-text`

A run of formatted prose, and the block an author gets by typing into an empty document.
...
The body is authored HTML, so the renderer treats it as untrusted on the way out. Shortcodes in it expand first. The renderer then sanitizes the whole result against the same allowlist `core/html` uses, and that is what reaches the page. The block also answers to the keywords `paragraph`, `text` and `body`, so typing `/paragraph` in the editor finds it.
...
The language field is a combobox rather than a select, so the stored value stays free text and content authored against an older list survives. Fifteen short forms map to a canonical id on the way in, `ts` to `typescript` and `py` to `python` among them. Highlighting runs synchronously inside the same render, which is why the server output and the editor canvas agree. A language the highlighter does not know keeps its `data-language` attribute and renders plain. The token colours ride along as one deduplicated stylesheet, themeable through the `--plumix-code-*` properties.
...
One input, `slug`. No category, and hidden from the inserter. An author writes a node of this shape by inserting a pattern that registered itself with `insert: "reference"`. The default is the other mode, which splices a copy of the pattern's body into the entry.

The walker handles this name before it consults the registry. It looks the slug up in the pattern registry and renders that pattern's blocks in place, which means editing the pattern changes every entry referencing it. An unresolved slug renders nothing on a production page, and in development logs the slug once and leaves a `<template>` marker.
...
Read [Overview](/blocks/overview/) if you have not, for the storage and render model these blocks sit inside.
