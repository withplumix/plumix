...
The label is the one option you can usually skip. Leave `.label()` off and the builder derives one from the key by splitting camel case and title-casing the result, so `prepMinutes` becomes "Prep minutes" and `heroImage` becomes "Hero image".
...
Ten methods appear on nearly every builder, and the type of the argument changes with the field.
...
`.default()` supplies a value for an absent key. It applies at decode time and seeds the admin form, and it never writes anything, so storage can still lack the key while a read gives you the default.

`.required()` marks the field required. The constraint walker enforces it on write, in `strict` mode only, so an autosave of an unfinished entry does not fail on it.
...
Each builder exposes the rule factories that suit its values. Every one has `.is()`, `.isNot()`, `.isEmpty()` and `.isNotEmpty()`. Numeric builders add `.gt()` and `.lt()`, `toggle` adds `.isOn()` and `.isOff()`, and a multi-value `select` adds `.contains()`, `.notContains()`, `.countGt()` and `.countLt()`. Registration rejects a rule naming a key no field in the box declares.
...
<Aside type="tip" title="Two shapes, not one">
  ...
  both what you read and what is stored, because the write is enforced.
  `.default()` narrows only what you read, because nothing was written. That
  split is why a builder tracks the two separately.
</Aside>
...
import { number, select, text } from "plumix/fields";
...
`meta.servings` is `number` because the field is required. `meta.difficulty` is `"easy" | "medium" | "hard"` because a default removed the `undefined`. `meta.unit` is `string | undefined`. The record is closed, with no index signature, so `meta.serving` is an error rather than `unknown`.
