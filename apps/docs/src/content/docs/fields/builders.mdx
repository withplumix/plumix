...
## Compiling a fields array yourself
...
<Aside type="caution" title="Builders come from plumix/fields">
  The builder functions are published at `plumix/fields` and nowhere else, and
  both helpers above sit beside them. `@plumix/core/fields` resolves and carries
  the same names, and it is one of the packages the façade covers, so it moves
  without warning. The general rule is in [Import from plumix, never from
  @plumix/core](/getting-started/project-structure/#import-from-plumix-never-from-plumixcore).
</Aside>
