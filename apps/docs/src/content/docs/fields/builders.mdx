...
`.span()` sets the field's width in the meta box's 12-column grid. `.capability()` gates the field on a capability string, which the server enforces on write. `.showInApi()` opts the value into public REST responses, which are default-deny per field. The `meta` map it feeds is keyed by top-level field, so the flag reaches it only from one — but an [image role](/fields/field-types/#image-roles) is addressed by name rather than by key, so a role field's own `showInApi` decides whether `images.<role>` appears, wherever the field sits. A role field inside a group opts its own image in, and does not inherit the group's answer in either direction. `.searchable()` does the same for the site's full-text index. It is honored on the text-shaped inputs — `text`, `textarea`, `email`, `url` and `richtext` — and ignored elsewhere, including on a `password` field and on a repeater row or group member, whose nested values are not walked.
...
<Aside type="tip" title="Two shapes, not one">
  ...
  match. `.default()` narrows only the read type: the decoder supplies the
  value, so a read never sees `undefined`, but nothing puts it in the column. A
  row saved before you added the field reads the default and stores nothing.
  That split is why a builder tracks the two separately, and why `.whereMeta()`
  — which compares against the stored bag — will not match a defaulted key until
  something saves it.
</Aside>
