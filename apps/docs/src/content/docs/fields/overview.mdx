...
Defaults do not apply. The decoder walks the keys storage actually holds, so an absent key reads back as `undefined` whatever `.default("medium")` says. A default prefills the admin form instead, and it reaches storage the first time someone saves that form. A template that needs a fallback for a key nobody has saved yet supplies one at the read site.
