...
That single declaration is also the whole contract. The admin reads it to pick an input control, the write pipeline reads it to coerce and validate the incoming value, the read pipeline reads it to apply defaults and resolve references, and the REST layer reads it to decide whether the value is public at all. Nothing repeats the declaration anywhere else, so a field cannot be right in one of those places and wrong in another.
...
Every value takes the same four steps, per field, in order.
...
2. **`sanitize`**, if the field declares one. It runs server-side only and its return value replaces the caller's input, which is where trimming and normalizing belong. Some field types inject their own: `range` gets one that rejects out-of-bounds values, and `link` gets one that rebuilds the value from its known keys so unrecognized properties never persist.
3. **Declarative constraints**, meaning the ones you wrote into the chain. `required`, `min` and `max`, `maxLength`, option membership, repeater row counts, the hex format on a `color`, the URL allowlist on a `url`.
4. **`validate`**, if the field declares one. Sync or async, returning `true` or a failure message the editor shows.

None of the four steps throws. Failures come back as `{ path, message }` pairs and the RPC layer aggregates them across the whole patch, so one save reports every bad field at once. The path is dot-joined from the meta key down into nested rows, so a bad amount in the third ingredient reports as `ingredients.2.amount`.

Strictness has two modes. `strict` is the default and enforces all four steps. `draft` skips the third and fourth, so an autosave of half-finished work never fails on a `required` field or an unmet bound. Coercion, shape normalization, `sanitize`, temporal validity and URL safety still run in `draft`, which is what stops an autosave persisting corrupt or unsafe data. Publishing re-runs the whole bag in `strict`.

## What a read gives back

A read is not the raw column. Three things happen on the way out.

Defaults apply at decode. A field with `.default("medium")` reads as `"medium"` when the key is absent, and storage still has no key, so the default is not silently written for you.
...
One thing does not happen. Meta stays out of the public REST API, which is default-deny per field, and a field opts in with `.showInApi()`. The admin RPC is unaffected.
