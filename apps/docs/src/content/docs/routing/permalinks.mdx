...
<Aside type="caution" title="Some scripts transliterate to nothing">
  ...
  returns an empty string for them. The entry editor's slug field requires at
  least one character, so it rejects the empty slug inline and asks the author
  to type one. The term create form has no such minimum, so it checks the name
  and the slug box together. Leave the slug blank on a name that transliterates
  to nothing and the form stops the submit before the RPC goes out and asks you
  to type a slug manually.
</Aside>
...
Six surfaces do pay for the ancestor walk and so escape the second case. The taxonomy resolver builds `data.term.url` for the term whose archive you are on with the async builder, and the sitemap composes every `<loc>` with it, so a nested entry has a sitemap URL even when its own `ResolvedEntry.url` is null. The feeds, the entry lookup and term lookup procedures behind the admin's reference fields and menu items, and the preview-link procedure call the async builders too. The walk is one recursive query, not one query per ancestor.
...
The sitemap is not one of the three. It builds every `<loc>` itself, from the site origin plus a freshly composed permalink, and never calls `canonicalUrl`. The two agree on ordinary entry and term pages because both compose the same registration and the same slug, not because one function produced both.
