...
`registerArchiveType(name, options)` registers a whole archive: its URL patterns, what the archive contains, and an optional `priority`. It compiles to the `custom` intent, and the theme targets it by name. `@plumix/plugin-seo` augments the same options with an optional `sitemap` and `@plumix/plugin-feeds` with an optional `feed`, so an installed plugin can index or syndicate the archive too. Reach for it when core has no concept of the archive at all, such as a printable weekly menu.

`entries` is what the archive contains, and declaring it is what hands the listing to core. It receives an entry query already holding nothing but public entries, narrows it, and returns it — or `null` for params that name no archive, which is a 404. It is synchronous and runs no queries; the term slug below is resolved when the page is served.

```ts
import { definePlugin } from "plumix/plugin";

export const talks = definePlugin("talks", {
  setup: (ctx) => {
    ctx.registerArchiveType("talks", {
      ...
      entries: (q, params) =>
        q
          .ofTypes("talk")
          .inTerm("track", params.track ?? "")
          .orderBy("title"),
      perPage: 10,
      title: (params) => `Talks: ${params.track ?? ""}`,
    });
  },
});
```

Core then lists that query, pages it, and reads the order it carries. It derives `/talks/:track/page/:page` from each declared route — you do not register the paginated form — and answers 404 past the last page. The theme receives `data.entries` and `data.pagination`, the same two fields a built-in archive hands over, so its pagination and entry components work unchanged. With `cacheable: true`, core tags the stored page with the entry types the query can list, and a publish of any of them purges it.

`perPage` defaults to 20. `title` takes a string or a function of the route params. `resolve` becomes optional once `entries` is declared, and is left with what only it can do — load a subject the route names, add fields of its own. It is handed the finished page as a third argument, and what it returns is merged under core's `entries` and `pagination`:

```ts
import type { AppContext } from "plumix";
import { definePlugin } from "plumix/plugin";

declare function loadRoom(
  ctx: AppContext,
  slug: string | undefined,
): Promise<{ name: string } | null>;

export const rooms = definePlugin("rooms", {
  setup: (ctx) => {
    ctx.registerArchiveType("rooms", {
      routes: ["/rooms/:room"],
      entries: (q) => q.ofTypes("talk"),
      resolve: async (appCtx, params, listing) => {
        const room = await loadRoom(appCtx, params.room);
        if (room === null) return null;
        return {
          data: { kind: "custom", name: "rooms", room: room.name },
          title: `${room.name} (${String(listing.pagination.total)})`,
        };
      },
    });
  },
...
An archive that leaves `entries` out works exactly as before: `resolve` returns the whole `{ data, title }` payload or `null`, and owns its own paging. That is the shape for an archive whose results are a match rather than a set — `@plumix/plugin-search` registers this way — and only an archive described by an entry query can have a feed.

### Narrowing an archive

`archive:entries` hands any archive's entry query to a plugin to narrow — core's front page, type, term, author and date archives as well as a plugin archive with `entries`. It is Plumix's `pre_get_posts` for archives. The handler receives the query, the archive it describes, and what the archive's URL captured, and returns a query. It runs wherever the archive's query is read, so the archive's page, its pagination and its feed all change together.
...
// Entries not filed under the `asides` category.
const notAnAside = sql`${entries.id} not in (select ${entryTerm.entryId} from ${entryTerm} inner join ${terms} on ${terms.id} = ${entryTerm.termId} where ${terms.taxonomy} = 'category' and ${terms.slug} = 'asides')`;

export const quietFrontPage = definePlugin("quiet-front-page", {
  setup: (ctx) => {
    ctx.addFilter("archive:entries", (query, archive) =>
      archive.kind === "front-page" ? query.where(notAnAside) : query,
    );
  },
});
```

`archive` names the archive as the router does — `front-page`, `archive` (with its `entryType`), `taxonomy` (with its `taxonomy`), `author`, `date`, or `custom` (with the plugin archive's `name`). No term or author is loaded for it; read the slug off the params. A query only narrows, and core applies the public-entries rule again when it compiles one, so a handler can hide entries but never reveal one — even returning a fresh `entryQuery()` lists no draft. A handler can change the order too, and a feed still reads newest first. Like `entries`, the handler is synchronous and runs no queries. The search page is not an entry-query archive and does not pass through it.
...
The route answers `GET` and `HEAD`; a write method gets a 405 from the step above it. Without an `access` policy, the handler runs ahead of the access gate and the principal loader, so `ctx.user` is null however the request was authenticated — a handler that lists content is listing it for an anonymous reader and has to leave out what an anonymous reader may not see.

Pass `access` — the same [access policy](/access/overview/) `registerArchiveType` takes — to gate the route instead. Plumix loads the principal and resolves the policy before the handler runs. A reader the gate refuses gets the gate's own response, a redirect to sign-in or a challenge, and the handler never runs. A reader it admits reaches the handler with `ctx.user` loaded and `ctx.access` set. A policied route answers per reader, so it never enters the CDN, even with `cacheable: true`, and a response for any segment other than `anonymous` is sent `private, no-store`.

The request URL has had any `basePath` stripped, so build outbound URLs from `ctx.origin` and `ctx.basePath` rather than from `request.url`.
