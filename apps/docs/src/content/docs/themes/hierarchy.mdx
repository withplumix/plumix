...
1. **Cover the tiers first.** A theme with `fallback` alone is already complete, because every node reaches it. Add tiers as the pages diverge.
   ...
   import { archive, defineTemplate, entry, fallback } from "plumix";
...
2. **Add a targeted rule above them.** Order matters among targeted rules, so put the narrower one first.
   ...
   import { defineTemplate, entry, forEntryType } from "plumix";
...
3. **Check what won.** Run the dev server and open `/recipes/sicilian-caponata`. The debug bar's Template panel names the winning rule and lists every rule the walk passed over, with the reason.
...
The trace is dev-only work. On a live render the resolver does the plain walk and builds no explanation.

## Related

[Templates](/themes/templates/) is the roster of the builders these rules come from, including every selector a targeted matcher can carry. [Template Data](/themes/template-data/) is the other half of what the resolver produces, the shape your render function reads. [Overview](/themes/overview/) covers the theme descriptor the `templates` array sits in.
