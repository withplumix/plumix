...
The selectors themselves you do not write. `entryTypeTargets` and its four siblings mint every matcher the framework knows how to compare, and they take your bind function so the chain ends in _your_ terminal — `.template(...)`, `.define(...)`, whatever you named it.
...
It is therefore absent from every constructor above, and reaching for it is a compile error:
...
If what you want is an author-selectable choice, the mechanism underneath `named` is available to you: add a field to a [meta box](/fields/meta-boxes/) and narrow on it with `.where()`. That gives you the same predicate against a key you own, and a place in the admin where the author actually sets it.
