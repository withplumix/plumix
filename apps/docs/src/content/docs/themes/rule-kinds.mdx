...
The predicate half: matches when a content entry's meta value equals `value`.
...
The comparison is `===` against `data.entry.meta[key]` — the bag the resolved entry carries, which is decoded rather than raw: a `.returns("date")` field is a `Date` there, and a reference is whatever its lookup adapter hydrated rather than the id the JSON holds. Compare against those, not against what you would see in the meta column.
...
The same predicate for terms, read off the meta a resolved term carries.
