...
For a single branch, the guards below read better. Each is a plain function exported from `plumix/theme` that takes `TemplateData` and narrows.

Three projections recur across the shapes.

- `ResolvedEntry` is the stored entry row with `meta` replaced by its decoded counterpart, with every [reference field](/fields/field-types/) hydrated, plus `storedMeta`, `images`, `terms`, `author`, `contentBlocks` and `url`. `storedMeta` is the meta JSON column as the row holds it, kept beside the decoded bag because that is what a rule predicate compares against — see [`.whereMeta()`](/themes/hierarchy/#predicates-run-second). `contentBlocks` is the content narrowed to the block-tree shape, and `null` when the stored JSON does not fit that shape. `url` is `null` for a hierarchical entry still awaiting an ancestor-chain walk.
- `ResolvedTerm` is the term row plus its own `url`, `storedMeta` and `images`, and its `meta` gets the same treatment as an entry's — decoded, with every [reference field](/fields/field-types/) hydrated. `storedMeta` is the meta JSON column as the row holds it, so the two differ on a term exactly as they do on an entry: a `.returns("date")` field reads a `Date` off `meta` and the stored ISO string off `storedMeta`, and a reference reads the row it points at off one and the stored id off the other. A rule predicate compares against `storedMeta` — see [`.whereMeta()`](/themes/hierarchy/#predicates-run-second).
- `ResolvedAuthor` is `id`, `slug`, `name`, `avatarUrl` and `images`. There is no email and no auth column, and the meta bag the images came from does not travel with them. On the entry path the query selects those columns and nothing else; on an author archive the resolver loads the whole user row to find the author, then copies the same fields into a fresh object rather than spreading the row.

All three carry `images`: the entity's picture for each [image role](/fields/field-types/#image-roles) its scope declares a field in, keyed by role name and typed from `ImageRoles`, so a template asks for `data.entry.images.featured?.url` rather than guessing a meta key. A role reads `null` when no field in it resolved to an image — an orphaned reference, or one the media adapter refuses — and is absent when the scope declares no field in it at all. The images are projected out of the meta the page already hydrated, so reading them costs no query.
...
import { defineTemplate, isEntry } from "plumix/theme";
...
```tsx
import { defineTemplate, isArchive } from "plumix/theme";
...
```tsx
import { defineTemplate, isTaxonomy } from "plumix/theme";
...
```tsx
import { defineTemplate, isAuthor } from "plumix/theme";
...
```tsx
import { defineTemplate, isDate } from "plumix/theme";
...
```tsx
import { defineTemplate, isCustom } from "plumix/theme";
...
Core only ever sees the base shape: `kind`, `name`, and two optional facts an archive states about itself — `page`, the 1-based pagination index, and `query`, what the visitor typed on an archive that answers a search. Both are what a consumer such as `@plumix/plugin-seo` classifies the page by; core cannot derive either from the rest of the payload. The [plugin](/plugins/overview/) that called `registerArchiveType` extends it with whatever its archive lists and declares the extended type in `ArchiveTypeRegistry`, so `forArchiveType("seasonal")` types `data` as the plugin's shape rather than this one. Reached through `fallback` instead, it stays the base, and the guard is how you find out which archive you are on.
...
```tsx
import { defineTemplate, isFrontPage } from "plumix/theme";
...
```tsx
import { defineTemplate, isSearch } from "plumix/theme";
...
```tsx
import { defineTemplate, isError } from "plumix/theme";
