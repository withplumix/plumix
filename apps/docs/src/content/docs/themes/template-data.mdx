...
- `ResolvedTerm` is the term row plus its own `url` and `storedMeta`. Its `meta` does not get the entry treatment. The public render path hands you the raw JSON column, so a reference field on a term arrives as the stored id rather than the row it points at — which makes `meta` and `storedMeta` the same bag on a term today, where they differ on an entry.
