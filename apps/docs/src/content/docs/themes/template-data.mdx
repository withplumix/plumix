...
All three carry `images`: the entity's picture for each [image role](/fields/field-types/) its scope declares a field in, keyed by role name and typed from `ImageRoles`, so a template asks for `data.entry.images.featured?.url` rather than guessing a meta key. A role reads `null` when no field in it resolved to an image — an orphaned reference, or one the media adapter refuses — and is absent when the scope declares no field in it at all. The images are projected out of the meta the page already hydrated, so reading them costs no query.
