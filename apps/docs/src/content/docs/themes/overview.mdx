...
- `tokens` are named design values, grouped by CSS property, emitted as custom properties on the page. The known groups are `color`, `spacing`, `fontFamily`, `fontSize`, `fontWeight`, `lineHeight`, `letterSpacing`, `borderWidth`, `borderRadius`, `boxShadow`, `textShadow`, `backgroundImage` and `maxWidth`, and a theme may add a group for any other CSS property.
...
A template may also carry `document`, either a literal manifest merged over the theme's at boot or a function called per request with the same argument `render` sees. That is how one page contributes an `og:image` without every page paying for it.
...
It is the smallest theme there is: one `fallback` rule, one `defineTemplate`, no tokens and no stylesheet. The rendered screen inlines its CSS rather than linking one, so it fetches nothing, and it reads `ctx.locale.code` to pick its strings. English, German, Ukrainian, Arabic and Simplified Chinese ship; any other locale renders English. It carries a link to `/_plumix/admin`, built through `ctx.basePath` so it stays right on a site served from a subdirectory.
...
Four further pages in this section are not written yet, and what they cover already ships. Document Manifest goes through `DocumentManifest` and the `theme:document` boot-time filter that lets a plugin contribute head tags. Tokens and Breakpoints covers the token groups above and the custom properties they emit. Component Primitives documents `Link`, `Image`, `PlumixProvider`, `useAuth` and the signed-in and signed-out wrappers. Template Dependencies covers declaring a dep kind on a theme or a template, and the override and extend forms.
