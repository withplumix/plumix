...
One caveat on `rewrite.slug`: keep it to a single path segment. A slug containing `/` compiles and the archive page renders, but the feed router reads only the first segment when it matches `/<taxonomy>/<term>/feed`, so term feeds under a nested base stop resolving.
