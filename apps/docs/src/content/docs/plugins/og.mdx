...
The plugin exports a factory named `og`. It adds no database tables, so installing it needs no migration, and it takes four options — `renderer`, `fonts`, `preview` and `palette` — each with a default. `renderer` and `fonts` are read together: a renderer declares which font formats it parses, and the plugin hands it no others.
...
`<target>` names the page a card is about — `entry/12`, `term/3`, `archive/post`, `author/7`, `date/2026-03`, `front-page` — and each of those six gets a card from the bundled template with nothing configured. See [which page kinds have a card](#which-page-kinds-have-a-card) for when each one is served.

<Aside type="note" title="Two page kinds have no card">
  A search page and a `registerArchiveType` archive get none: a search page's
  subject is whatever the visitor typed, so a card per query would fill your
  bucket from the query string, and a plugin archive is resolved by its own
  plugin from route parameters no card URL can name. `card.search()` and
  `card.forArchiveType()` rules still type-check and resolve — they are simply
  never addressed.
</Aside>
...
```sh
npm install @plumix/plugin-og @plumix/plugin-seo
```
...
```sh
yarn add @plumix/plugin-og @plumix/plugin-seo
```
...
```sh
bun add @plumix/plugin-og @plumix/plugin-seo
```
...
1. **The entry's `ogImage` [image role](/fields/field-types/#image-roles).** An author who picked a share image gets it, untouched.
2. **Its `featured` role** — the entry's own photo — **cropped to the card's size** through your `imageDelivery:` slot. A photo shot at 4:3 unfurls letterboxed or badly cropped in a 1.91:1 slot, and this is what fixes it. The crop is URL math, so it costs no CPU and reaches no rasterizer. With no delivery slot configured the photo is emitted as it stands rather than dropped.
...
<Aside type="caution" title="A listing card renders from page one">
  A card names the archive, not one paginated slice of it — `/posts/page/2`
  advertises the same card `/posts` does. So `data.entries` in a listing card is
  always the archive's first page, whichever page is being viewed, and keying a
  card on `data.pagination.page` names something the route never renders.
</Aside>
...
**Which formats are read is your renderer's business, not this option's.** A renderer declares what it reads beside what it writes, and the plugin fetches only the configured faces it can parse, in the order you listed them.

| Renderer                         | Reads                                            |
| -------------------------------- | ------------------------------------------------ |
| `takumi()`, `svgOnly()`, default | TTF, OTF and WOFF — **not** WOFF2                |
| `remote({ url })`                | nothing: the endpoint brings its own faces       |
| `remote({ url, fonts })`         | whatever formats you declare the endpoint parses |

WOFF2 is what most font packages ship by default, and the bundled engine cannot read it. Configure a set the connected renderer can parse none of and the card fails with an error naming the format, rather than serving a correct render of a font with no glyphs.

A renderer that reads no fonts ignores `fonts` entirely — nothing is fetched, nothing is sent, and a runtime with no asset layer at all still serves every card. The setting is then dead config, which the debug bar's **OG image** panel points out in development. It also drops out of the card's URL: a card's digest names only the faces its renderer actually receives, so swapping a font path moves no URL on a site rendering off-box.

### Sending your faces to a remote endpoint

An endpoint that would rather render in your site's own faces asks for them by declaring what it parses:

```ts no-typecheck
import { og, remote } from "@plumix/plugin-og";

og({
  fonts: ["/fonts/Inter-SemiBold.woff2"],
  renderer: remote({
...
Faces then travel base64 in the request body, beside the images that already do — never as URLs, because an endpoint handed a URL has to fetch it, which is the class of problem this plugin removes rather than mitigates. The bill is the reason it is opt-in: several hundred KB per face on every render, and several MB for a CJK face.
...
### A newer engine does not arrive on its own
...
The `featured` and `ogImage` roles that outrank a card are filled by meta-box fields — [Meta Boxes](/fields/meta-boxes/) covers declaring them and [Image Roles](/fields/field-types/#image-roles) what a role is and how to add one of your own. Whether an entry may carry a card at all follows [Statuses and Publishing](/content-modelling/statuses/) and the model in [Access & Identity](/access/overview/).
