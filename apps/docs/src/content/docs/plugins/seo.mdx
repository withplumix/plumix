...
When a social image resolves, `og:image` is emitted with `og:image:width`, `og:image:height`, `twitter:image` and — where the image says what it shows — `og:image:alt` and `twitter:image:alt` beside it. They travel as a group: a theme that declared its own `og:image` keeps the whole group, because a size or a description appended next to it would describe some other picture.

The alt text is the media row's own: an image chosen through the `featured` or `ogImage` role carries it, so filling in a picture's alt text in the media library is what puts it in the head. A generated card, a URL typed into the SEO box and the site-wide default have none, and those pages carry neither alt tag rather than an empty one.

## The social image chain

Five links resolve one `og:image`, in this order:

1. **The entry's explicit choice** — the entry's `ogImage` [image role](/fields/field-types/#image-roles).
...
4. **The entry's featured photo** — its `featured` role.
...
An image a subscriber returns may carry an `alt` of its own, which is then the one the head emits — a crop of the author's photo still shows what the photo showed, so passing its alt along keeps the description with the picture.
...
| `%%searchphrase%%` | The query, on a search-results page — core's own, or a plugin archive that states one.             |
| `%%count%%`        | How many entries the listing matched, on an archive core paginates.                                |
...
ctx.addFilter("seo:schema:needs", (needed, piece, facts) =>
  piece === "breadcrumb" && facts.query !== null ? false : needed,
);
```
...
An entry's pictures ride its `<url>` as [sitemap image entries](https://developers.google.com/search/docs/crawling-indexing/sitemaps/image-sitemaps), so image search finds them without crawling the page for `<img>` tags. Those pictures are the entry's [image roles](/fields/field-types/#image-roles), one image per role its type declares a field in — the same roles link 1 and link 4 of the chain above read — plus the social image URL an editor typed into the SEO box, which leads because it is the one they chose. One photo named by two roles is listed once.

A whole page of entries resolves through one batched pass rather than a query each — chunked at 100 ids per query, as core's own read path chunks — and only for types that declare a role field. An entry with no picture serializes exactly as it did before, and a page with none declares no image namespace at all.

Only pictures are listed, and nothing here sniffs a payload to decide: the adapter that produced a reference answers whether it is an image, so a role field holding a PDF resolves to nothing. A role holds one image, so an entry lists at most one picture per role it declares. Relative URLs — the worker-proxied path a private bucket resolves to — are made absolute, since `<image:loc>` has to be.
...
Sitemap responses declare `public, max-age=0, s-maxage=3600` and go through the CDN when one is configured. Each scope stores under its own tags — `t:<type>` for an entry type, the `t:<type>` tags of a taxonomy's entry types for a taxonomy — so publishing a post retires the post scope and leaves the rest of the set alone. Saving the settings group purges all of them, since the indexing toggle decides whether any of them have URLs at all.
...
- One **title pattern** and one **indexing toggle** per public entry type, and one **indexing toggle** per public taxonomy. Those fields are enumerated from what the site registered, so a type a plugin registers during its own `setup` gets its own. A type registered later — from the `afterSetup` of a plugin listed after this one — is too late to be enumerated, and a name that is not `[a-zA-Z0-9_-]+` cannot be a settings key, so it gets no per-type fields either.
...
The `featured` and `ogImage` roles the chain reads are filled by meta-box fields — [Meta Boxes](/fields/meta-boxes/) covers declaring them and [Image Roles](/fields/field-types/#image-roles) what a role is and how to add one of your own. The head this plugin writes into is the theme's: [Themes](/themes/overview/) covers the `document` declaration whose tags win, and [Template Data](/themes/template-data/) the `data` a `seo:og_image` subscriber receives.
