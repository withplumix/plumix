...
Three admin screens drive them. The login screen calls the two `login/*` endpoints. The bootstrap screen and the profile Passkeys card both call the two `register/*` ones. The accept-invite screen calls the invite pair. You do not call any of them directly unless you are building a sign-in page of your own.
