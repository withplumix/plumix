...
>
  The generated config pins the passkey origin to the port above. WebAuthn
  compares that string against the origin the browser reports, so running on a
  different port fails the ceremony with an origin mismatch. Change `origin`
  (Node) or `localOrigin` (Cloudflare) in `plumix.config.ts` to match whichever
  port you run.
</Aside>
