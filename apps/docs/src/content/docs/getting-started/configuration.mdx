...
`plumix()` returns a resolved config rather than the object you passed. Four slots gain a default on the way through. A missing `theme` becomes the built-in welcome theme, a missing `plugins` becomes an empty array, a missing `redirects` becomes an empty array, and a missing `i18n` becomes English only. `basePath` is normalized, so `docs`, `/docs` and `/docs/` all resolve to `/docs`.
...
The slots that take an `EnvInput` are `mailer`, the passkey `origin` and `allowedOrigins`, and each OAuth provider's client credentials. Locally their values come from `.dev.vars`; in production they are secrets on the deploy.
...
<Aside type="caution" title="T has to be a non-callable value">
  ...
  "function"`. Every slot that accepts an `EnvInput` today resolves to an
  object, so the test is unambiguous. A slot whose value were itself a function
  could not use this form.
</Aside>
...
Sign-in and session policy. `passkey` is required inside it, and everything else is opt-in: `oauth` providers, `magicLink`, a `sessions` policy, a replacement `authenticator`, `selfSignup`, `bootstrapVia` and `loginPath`. `auth()` validates its input and throws on a bad shape rather than deferring to the first sign-in.
...
Object storage for uploads. `r2` binds a bucket; the media plugin requires this slot and `imageDelivery` alongside it.
...
A rule with no `status` is a 301. The request's query string is appended to the target unless the target carries its own `?`, or you set `preserveQuery: false`. Config rules merge ahead of plugin rules and theme rules, so the site wins a tie.
...
The Model Context Protocol endpoint at `/_plumix/mcp`, where an MCP client reaches the site's tools. Default-off, and the gate is early. With no `mcp` block the dispatcher answers 404 before it imports the handler graph at all.
...
Under all four options is a floor no override can widen past. Script-capable URL schemes, `on*` event attributes and `style` stay denied whatever you write here.
