...
export const cdn = edge({ ttl: 3600, staleWhileRevalidate: 86400 });
```

`edge()` disables itself when the deploy lacks the zone id and purge token it needs, because a CDN that cannot purge would serve a stale recipe forever. That is why the CDN is inert on a `workers.dev` host and live on a zone.
