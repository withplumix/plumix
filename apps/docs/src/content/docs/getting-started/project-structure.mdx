...
Four scripts, each a thin wrapper over the CLI: `plumix dev`, `plumix build`, `tsc --noEmit`, and a `clean` that removes the generated directories. The dependencies are `plumix`, the runtime adapter, `react`, and whichever plugin packages you ticked.
...
The runtime adapter is the exception you will meet immediately. `@plumix/runtime-cloudflare` is a direct dependency of your project and a direct import, because a runtime is a thing you choose. `cloudflare`, `d1`, `r2`, `images`, `kv` and `edge` all come from there.
...
`definePlugin` and the registration context, plus valibot re-exported as `v` so a plugin can declare its own config schema without a second dependency.
...
`defineTheme`, `defineTemplate`, and the document-manifest types. The template builders themselves come from the root specifier.
...
The shared component library the admin renders, and the one admin subpath you do import by hand. Unlike the shims it ships real source, which the plugin-chunk build bundles into your chunk with its own React and Radix imports aliased to the shims. A wrapper costs about a kilobyte; Radix itself is not duplicated.
...
The meta-box field builders. The scalar builders are fluent and typed so that a chain which does not apply is a compile error; the reference factories take flat options.
