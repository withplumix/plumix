...
The project installs TypeScript 6. TypeScript 7 works too, since nothing in the project uses the compiler API that 7.0 dropped: `typecheck` is plain `tsc`, and Plumix parses your sources with Vite's own parser. Run `pnpm add -D typescript@7` to type-check with the native compiler. Tools that embed TypeScript themselves, such as typescript-eslint, still need TypeScript 6 until they support 7.
...
<Aside type="note" title="One specifier for type augmentation">
  ...
  declarations stop being visible. The file that holds the block has to import
  something from `plumix` itself (a type is enough), or TypeScript reports the
  module as not found.
</Aside>

## Writing the site

Every value has one import path, picked by who writes against it: the site's config, a theme file, a plugin, a runtime adapter. The path you type is the only one that works, so an editor's auto-import cannot pick a second one. Types are the exception. The root carries every one of them, and each type also sits on the subpath of the role that uses it, so a theme or plugin file needs a single import.

### plumix

The root, which is what `plumix.config.ts` writes: `plumix`, `defineConfig`, `consoleMailer`, `resolveEnvInput` and `PlumixConfigError`. It also carries every type, and it is the one specifier a `declare module` block targets.

```ts
import { consoleMailer } from "plumix";

// The `mailer` slot of `plumix({ … })`: a magic link prints to the terminal.
export const mailer = consoleMailer();
```

### plumix/plugin

What a plugin writes: `definePlugin` and its registration context, RPC procedures and hooks, entry and URL helpers, the responses a plugin route answers with, dev panels, and the pieces a plugin builds [its own rule kind](/themes/rule-kinds/) from. It also re-exports valibot as `v`, so a plugin can declare its own config schema without a second dependency.
...
What a runtime adapter needs to contribute CLI commands: `CommandDefinition`, the `CliError` the command dispatcher understands, and the `spawnInherit` / `spawnCapturingStderr` helpers a command shells out through. Narrow on purpose. Loading core's full runtime costs about 500ms of module evaluation, and a command author should not pay that before the CLI has parsed a flag.
...
What a theme file writes: `defineTheme`, `defineTemplate`, the [template builders](/themes/templates/) (`fallback`, `entry`, `forEntryType` and the rest), the `is*` guards that narrow a template's data, and the data types a template receives. The render components (`<Link>`, `<Image>`, `BlockRenderer`) are on [`plumix/blocks/renderer`](/getting-started/project-structure/#plumixblocksrenderer).
...
export const theme = defineTheme({ templates: [entry(recipe)] });
```

### plumix/auth

Identity and access: the `auth()` block and its providers (`github`, `google`), the authenticators a runtime composes a request's identity from, roles and capabilities, and the [access policies](/access/overview/) that gate content.

```ts
import { auth, github } from "plumix/auth";
...
export const recipeAuth = auth({
  ...
  oauth: {
    providers: {
      github: github({ clientId: "client-id", clientSecret: "client-secret" }),
    },
  },
});
```
...
What a runtime adapter composes an app from: `buildApp` and `createPlumixHandler`, scheduled tasks and their cron parser, query tracing, the in-memory `memoryKv` and `memoryStorage` a test or a local run stands in with, and the dev boot page. A site rarely imports it. The runtime adapter you install does.

```ts
import type { PlumixConfig } from "plumix";
import { buildApp, createPlumixHandler } from "plumix/runtime";

export async function handlerFor(config: PlumixConfig) {
  return createPlumixHandler(await buildApp(config));
}
```

### plumix/support

Small helpers any file can use: `escapeHtml`, `xmlEscape`, `slugify`, `nonEmpty`, `withBasePath`, `normalizeBasePath`, `escapeLikePattern` and the `isJsonObject` / `isJsonArray` narrowings. It never loads core's server runtime, so an admin chunk or an island can import it as well as a server file.

```ts
import { escapeHtml, slugify } from "plumix/support";
...
A direct write skips core's save path, and with it two things a save does for you. It does not purge the CDN, so enqueue the tags yourself with `entryPurgeTags` and `enqueuePurgeTags`. And it does not convert meta to the form its fields declare, so a `1` written under a `toggle` field reads back as `1`, not `true`. Pass the bag through `settleMeta` first. It takes who the bag belongs to, `{ entryType }`, `{ taxonomy }`, `{ settingsGroup }` or `"user"`, and applies the same conversion a save does. A value no field type accepts, and a key no field declares, come back as given, and so does the whole bag when nothing is registered under the name you pass. Settings are stored one row per key, so settle them as `{ [key]: value }` and write each key of the result to its own row.

```ts
import type { AppContext, JsonObject } from "plumix";
import { enqueuePurgeTags, entryPurgeTags, eq, settleMeta } from "plumix/db";
import { entries } from "plumix/schema";

// An importer replacing a recipe's meta with the bag it read from the source.
export async function importRecipeMeta(
  ctx: AppContext,
  id: number,
  meta: JsonObject,
) {
  await ctx.db
    .update(entries)
    .set({ meta: settleMeta(ctx, { entryType: "recipe" }, meta) })
    .where(eq(entries.id, id));
  enqueuePurgeTags(ctx, entryPurgeTags("recipe", id));
}
```
...
```ts
import { memoryKv } from "plumix/runtime";
