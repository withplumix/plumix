...
<Aside type="note" title="Cloudflare is the reference implementation">
  `@plumix/runtime-cloudflare` is the only adapter that ships today, and it is
  short, because the default handler does the rest. Read it beside this page.
  [Cloudflare Workers](/deployment/cloudflare/) documents it from the operator's
  side.
</Aside>
