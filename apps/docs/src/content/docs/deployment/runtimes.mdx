...
An absent port member is a supported provider, not a gap the suite reports: a vendor that cannot invalidate by tag omits `purgeTags`. Publish one and recommend a short `ttl` beside it — [CDN Caching](/deployment/cdn/#providers-without-tag-purge) says why freshness is then the site's only control.
