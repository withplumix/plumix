...
Nothing fills `kv` or `cdn` yet; [What Node does not do yet](#what-node-does-not-do-yet) says what that means for a site.
...
There is no CDN and no `kv`, so a page renders on every request, and a plugin requiring either capability cannot be scaffolded on Node; the scaffolder says so by name. Both are single-node concerns, as are the uploads and the image cache: this runtime is one process on one machine.
