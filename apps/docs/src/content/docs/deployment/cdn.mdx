...
The platform's own endpoints decide for themselves and leave nothing to guess at. Every `/_plumix/rpc/**` and `/_plumix/api/v1/**` answer declares `no-store` — refusals included, since a 404 and a 405 are both heuristically cacheable and would otherwise be the responses an intermediary felt free to keep. A PAT-authed read adds `private` on top. Nothing on either surface carries a cache tag, so a shared copy of one could never be purged when the content behind it changes. The admin shell is the exception that proves the rule: it sends `private, no-cache`, which lets the visitor's own browser hold a copy it revalidates while `private` keeps it out of every cache in between.
