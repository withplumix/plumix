...
A raw route a plugin mounted at `/_plumix/<pluginId>` is the sixth cacheable kind, and it takes the same opt-in: `registerRoute({ cacheable: true })`. Only an `auth: "public"` route may take it, since a stored response is served to everyone, and registering it on a gated route throws at boot. A GET is answered from the entry, and every other method runs the handler. The entry is keyed off the request URL with the cookie dropped, so a signed-in visitor reads the shared copy rather than bypassing it the way a page render does — take the opt-in only where the route answers every visitor with the same document, locale included, since a `Vary` the handler sets is not a key axis here either. The whole URL is the key, query string included, so any parameter a caller invents is another entry. And a route entry stores untagged, because core cannot know what a plugin's handler depends on, so what expires it is its own freshness rather than a purge.
