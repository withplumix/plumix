...
Leave the flag off, on the purge argument above. It also caches more of the entrypoint than the `cdn` slot governs: every response the Worker returns passes through it, not only the ones the provider decorates. The platform's own endpoints are covered — RPC and the REST API declare `private, no-store` themselves, and the admin shell `private, no-cache` — so what is left undeclared is whatever a plugin's own route returns without saying. The scaffolded `wrangler.jsonc` does not set it.
