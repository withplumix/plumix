...
description: The Cloudflare runtime adapter, and the factories that fill the database, storage, KV, image and CDN slots with D1, R2, Workers KV, Image Transformations and the edge CDN.
tier: P0
sidebar:
  order: 20
---
...
| `edge({ ttl })`    | `cdn`           | The Workers Cache API and purge by tag   |
...
`edge({ ttl, staleWhileRevalidate })` stamps public pages with the freshness and cache tags a CDN reads, keeps a copy in the Workers Cache API, and purges by cache tag through the zone API. You add it to the config as `cdn: edge({ ttl: 3600, staleWhileRevalidate: 86400 })`. `ttl` becomes `s-maxage` in seconds, and `staleWhileRevalidate` is how long a colo may serve an expired copy while it refetches.
