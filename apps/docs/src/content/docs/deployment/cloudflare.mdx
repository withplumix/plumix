...
description: The Cloudflare runtime adapter, and the factories that fill the database, storage, KV and image slots with D1, R2, Workers KV and Image Transformations.
tier: P0
sidebar:
  order: 20
---
...
Cloudflare Workers is a fully supported runtime, chosen with one flag (`--runtime cloudflare`), and the one the CDN, KV and image transformations exist for. `@plumix/runtime-cloudflare` holds the adapter that bridges the Worker to Plumix, plus one small factory per platform service, and each factory goes in the config slot named after the job it does.
...
The CDN slot comes from core rather than from this package, because nothing about it is Workers-specific: `cloudflare({ ttl, staleWhileRevalidate, zoneId, purgeToken })` from `plumix/cdn/cloudflare` stamps public pages with the freshness and cache tags a CDN reads, keeps a copy in the Workers Cache API, and purges by cache tag through the zone API. You add it to the config as `cdn: cdn({ ttl: 3600, staleWhileRevalidate: 86400, zoneId: (env) => env.CF_ZONE_ID, purgeToken: (env) => env.CF_CACHE_PURGE_TOKEN })`, importing it as `import { cloudflare as cdn } from "plumix/cdn/cloudflare"` — every provider exports its bare vendor name, and the alias is what makes swapping one a single-word edit. `ttl` becomes `s-maxage` in seconds, and `staleWhileRevalidate` is how long a colo may serve an expired copy while it refetches.

A cacheable public page leaves the origin carrying those headers, not just the copy the Workers Cache API holds. Two responses leave untouched and untagged: one that declared itself `private` or `no-store`, and one that sets a cookie — that cookie is the visitor's own and cannot be stripped the way the stored copy's is. A response that declared its own shared-cacheable freshness keeps it; the `ttl` above applies only to one that declared none. Note that Cloudflare does not cache HTML without a cache rule on the zone, so the headers alone will not fill the edge cache until you add one.

`zoneId` and `purgeToken` are required fields, and the resolver form above keeps the secret out of the committed file. The slot disables itself when either resolves to nothing on this deploy. That is not a convenience check. Without a zone and a purge token the site could store a page it can never purge, so caching stays off and every page renders live. This is why a `workers.dev` deploy sees no caching even with the slot configured.

Four built-in route kinds are cacheable: a single entry, an archive, a taxonomy archive and the front page. Search is excluded, because one cache entry per query string pollutes the CDN for no gain. A custom archive that a plugin registered is the fifth kind, and it caches only when that registration passed `cacheable: true`, since core cannot work out what a plugin's archive depends on.

A request that carries a session cookie, an `Authorization` header or a `?preview=` token bypasses the CDN entirely. `?plumix.edit` is checked separately, on a route that carries an access policy, where the resolved audience segment would otherwise be cacheable. On a route with no policy the editor request is already caught by its session cookie. Stored responses have their `Set-Cookie` headers stripped before they are written.

A raw route a plugin mounted at `/_plumix/<pluginId>` is the sixth cacheable kind, and it takes the same opt-in: `registerRoute({ cacheable: true })`. Only an `auth: "public"` route may take it, since a stored response is served to everyone, and registering it on a gated route throws at boot. A GET is answered from the entry, and every other method runs the handler. The entry is keyed off the request URL with the cookie dropped, so a signed-in visitor reads the shared copy rather than bypassing it the way a page render does — take the opt-in only where the route answers every visitor with the same document, locale included, since a `Vary` the handler sets is not a key axis here either. The whole URL is the key, query string included, so any parameter a caller invents is another entry. And the tags are the handler's to name: core cannot know what a plugin's handler depends on, so a route stores untagged unless the handler calls `tagCdnEntry(ctx, entryPurgeTags(type, id))` while it runs, which puts its entry under the same tags the entry's own pages carry and clears it on the same publish.
...
Freshness is the response's to declare. A handler that sets its own `Cache-Control` keeps it — a content-addressed asset asking for `max-age=31536000, immutable` is stored with exactly that, and a content-addressed URL is the one kind `immutable` belongs on, since a purge reaches Cloudflare but never the browser or the scraper holding the URL — and the `ttl` above applies only to a response that set none. The Workers Cache API copy is the one place `private` and `no-store` are overwritten with the page TTL rather than obeyed: a segment variant sends them so no intermediary reuses its render, while the segment-keyed edge copy beside it is shared on purpose. The copy the visitor receives always keeps them.
