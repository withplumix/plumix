...
A raw route a plugin mounted at `/_plumix/<pluginId>` is the sixth cacheable kind, and it takes the same opt-in: `registerRoute({ cacheable: true })`. Only an `auth: "public"` route may take it, since a stored response is served to everyone, and registering it on a gated route throws at boot. A GET or HEAD request is answered from the entry, and any other method dispatches live. The entry is keyed off the request URL with the cookie dropped, so a signed-in visitor shares it rather than bypassing it the way a page render does — take the opt-in only where the route answers every visitor with the same document, locale included. The whole URL is the key, query string included, so any parameter a caller invents is another entry. And a route entry stores untagged, because core cannot know what a plugin's handler depends on, so what expires it is its own freshness rather than a purge.

Freshness is the response's to declare. A handler that sets its own `Cache-Control` keeps it — a content-addressed asset asking for `max-age=31536000, immutable` is stored with exactly that, and since no purge reaches it, that is the one kind of URL `immutable` belongs on — and the `ttl` above applies only to a response that set none. A plugin route that answers `private` or `no-store` is not stored at all, which is how a handler keeps one personalized answer out of the shared entry. A page render is the one case where those two directives are overwritten with the page TTL rather than obeyed: a segment variant sends them so no intermediary reuses its render, while the segment-keyed edge copy beside it is shared on purpose.
