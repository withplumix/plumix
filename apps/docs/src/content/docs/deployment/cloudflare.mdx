...
`edge({ ttl, staleWhileRevalidate })` caches public pages in the Workers Cache API and purges them by cache tag through the zone API. You add it to the config as `cdn: edge({ ttl: 3600, staleWhileRevalidate: 86400 })`. `ttl` becomes `s-maxage` in seconds, and `staleWhileRevalidate` is how long a colo may serve an expired copy while it refetches.
