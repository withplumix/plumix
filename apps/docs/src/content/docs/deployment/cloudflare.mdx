...
Cloudflare Workers is a fully supported runtime, chosen with one flag (`--runtime cloudflare`), and the one the edge cache, KV and image transformations exist for. `@plumix/runtime-cloudflare` holds the adapter that bridges the Worker to Plumix, plus one small factory per platform service, and each factory goes in the config slot named after the job it does.
