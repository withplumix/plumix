...
Leave the flag off. It caches more of the entrypoint than the `cdn` slot governs: every response the Worker returns passes through it, and only the ones the provider decorates carry a directive saying what a shared cache may do with them. The scaffolded `wrangler.jsonc` does not set it.
