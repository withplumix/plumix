...
These fail soft: an incomplete R2 credential set leaves presigned uploads off, and a CDN whose `zoneId` or `purgeToken` resolves to nothing leaves the CDN off. You get a working site with one capability absent rather than a 500, which is convenient and easy to miss. Check the capability, not the deploy log.
