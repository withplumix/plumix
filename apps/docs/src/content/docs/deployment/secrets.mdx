...
All of these fail soft. A missing pair leaves the CDN off; an incomplete R2 credential set leaves presigned uploads off. You get a working site with one capability absent rather than a 500, which is convenient and easy to miss. Check the capability, not the deploy log.
