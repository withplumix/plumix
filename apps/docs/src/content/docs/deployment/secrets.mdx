...
Give the recipe site a real mailer, so magic-link sign-in sends mail instead of logging it.
...
2. **Write the resolver.** The function receives the typed env and returns the value the slot wants.
   ...
   Pass it as `mailer: resendMailer` in `plumix.config.ts`.
...
Restart the dev server after editing it. The file is read at boot.
...
`wrangler secret put NAME` is the only route in. A secret must not go in the wrangler `vars` block, because that block is part of the config file and gets committed.

Wrangler can hold you to it. Listing a name under `secrets.required` makes `wrangler deploy` fail when the secret is not configured, which turns a silently broken feature into a failed deploy:
