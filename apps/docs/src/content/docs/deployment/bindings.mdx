...
Three adapters look for particular env keys, and not in the same way. For `r2()` and `images()` the env is a fallback the config can beat. For `edge()` it is the only route in, because `EdgeConfig` takes `ttl` and `staleWhileRevalidate` and nothing else. Set the keys as vars or as secrets, whichever the value warrants.
...
Passing the value in config beats the env key, for the two adapters that offer the choice. `r2({ publicUrlBase })` and `images({ zone })` both win over what the env carries. `edge()` has no such option, so `CF_ZONE_ID` and `CF_CACHE_PURGE_TOKEN` have to be on the env.
