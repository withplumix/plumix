...
Read [Configuration](/getting-started/configuration/) for the MCP endpoint's own dev relaxation and the `dev` options, and [Deploy](/getting-started/deploy/) for what changes when the same worker is built rather than served.
