...
- Out of `published` back to `draft`: an ordinary update, with no publish capability and no strict validation. The other direction is the publish transition in the first bullet.
