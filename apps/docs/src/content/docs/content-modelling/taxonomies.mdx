...
`entryTypes` on the taxonomy decides what a term archive contains and what a term edit purges. The archive at `/cuisine/italian/sicilian` lists entries whose type appears in that array and is public. A taxonomy registered with no `entryTypes` lists the entries of every public type carrying the term. The same set of types drives cache purging, so editing the Sicilian term purges the stored pages of every entry type the taxonomy classifies.
