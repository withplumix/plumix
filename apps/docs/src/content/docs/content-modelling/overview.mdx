...
3. **Start the dev server.**
   ...
   Recipes now has its own section in the admin sidebar, with a list page and an editor. Publish one and it serves at `/recipes/sicilian-caponata`, under an archive at `/recipes`.
...
You declare the pairing from both sides, and each side does a different job. `termTaxonomies` on the entry type puts the cuisine and diet pickers on the recipe editor. `entryTypes` on the taxonomy is what the cache purge reads, so editing the Sicilian term purges the recipe pages carrying it.
