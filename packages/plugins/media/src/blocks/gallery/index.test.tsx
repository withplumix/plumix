...
import { imageBlockV2 } from "../image/v2.js";
import { galleryBlockV2 } from "./v2.js";

describe("media/gallery v2", () => {
  test("renders an empty grid with default 3 columns", () => {
    const html = renderBlockSpecToHtml(galleryBlockV2, {});
  ...
  test("emits data-aspect when a valid ratio is provided", () => {
    const html = renderBlockSpecToHtml(galleryBlockV2, { aspect: "16:9" });
    expect(html).toContain('data-aspect="16:9"');
  });

  test("omits data-aspect when the value is missing or invalid", () => {
    const empty = renderBlockSpecToHtml(galleryBlockV2, {});
    const bogus = renderBlockSpecToHtml(galleryBlockV2, { aspect: "nope" });
  ...
  test("clamps invalid column counts into the supported range", () => {
    const tooMany = renderBlockSpecToHtml(galleryBlockV2, { columns: 99 });
    const tooFew = renderBlockSpecToHtml(galleryBlockV2, { columns: 0 });
  ...
  test("renders nested media/image children from the content slot", () => {
    const html = renderBlockTreeToHtml([galleryBlockV2, imageBlockV2], [
      {
        id: "g1",
        name: "media/gallery",
        attrs: {
