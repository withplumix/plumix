...
- **A settings group** — the site-wide indexing toggle (which drives the robots directive on every page, `robots.txt` and the sitemap), the default social image, and whether the site represents an organization or a person, on a settings page of its own.
