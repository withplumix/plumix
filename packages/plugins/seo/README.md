...
- **`/robots.txt`** — allow-all while indexing is on, disallow-all when it is off, editable from the admin, always carrying a `Sitemap:` line, and adjustable through the `seo:robots-txt` filter. Turning **Block AI crawlers** on adds one group disallowing the crawlers that feed model training and assistant answers, leaving ordinary search crawlers alone.
...
- **A settings page** — three groups, each gated by `settings:manage`: the site-wide indexing toggle (which drives the robots directive on every page, `robots.txt`, the sitemap and the `llms.txt` map), the default social image, whether the site represents an organization or a person, the AI-crawler toggle, the IndexNow key and the per-type answers; search-engine verification tokens; and the `robots.txt` editor.
