...
// `SHARED_ADMIN_RUNTIME_SPECIFIERS` const lives in `@plumix/core` so admin
// (which only depends on core) and the consumer-side vite plugin can both
// reach it without depending on plumix itself; we re-export here so
// plugin-author docs can point at a single `plumix/admin` surface.
//
// We import those constants from the browser-safe `@plumix/core/admin`
// subpath, not the root barrel — `getRuntime` below is browser-facing, so
// this module can land in a plugin chunk (see `@plumix/core/admin` for why
