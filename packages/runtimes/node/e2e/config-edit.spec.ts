...
// evaluates the config through its own module runner, so an edit is only
// served if the runner's cache is invalidated along the import chain
// (`invalidateFile` in `src/commands/dev.ts`). Cloudflare gets the same
// effect from wrangler restarting the worker, which is why the shared spec
// does not carry this.
//
// The cost is that this is the one e2e side effect landing outside the wiped
// `data/`: it rewrites a git-tracked source file.
...
// An edit and a restage each take seconds on a loaded runner, and this test
// drives two of them.
