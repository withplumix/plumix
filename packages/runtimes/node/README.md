...
The entry serves `dist/client` from disk, then image transforms, ahead of the site; it listens on `PORT` (3000) and `HOST` (`0.0.0.0`), and prints the bound address once. On `SIGTERM` or `SIGINT` it stops accepting and gives the shutdown one 10-second budget: in-flight responses keep finishing from that moment, the scheduler stops first if cron is running, and deferred work drains in whatever time is left. If the scheduled run, the responses and the deferred work all settle in time the process exits 0; otherwise it exits 1, logging which of them the budget cut, with a count of abandoned deferred tasks. A second signal exits at once. Importing the module instead of running it starts no server, and its two entry points do not serve the same paths. `listener(req, res)` is that stack as Connect-style middleware. `dispose()` drains the site's deferred work and releases its database connection; a host embedding `listener` owes the site that call on its own shutdown. The default export is the bare `{ fetch, scheduled }` handler and runs neither layer, so `/assets/*`, `/_plumix/image` and the admin's own `/_plumix/admin/assets/*` go unserved — a deep link returns the admin shell, but its chunks 404. Embed `listener` unless your host serves those paths itself. Either entry point answers every request it receives, so mount it where the site should own the path.
...
What the generated entry calls, and everything it does beyond importing. `handler` is the portable `{ fetch, scheduled }` pair — `scheduled` answers with the run's report, which is what says a task failed. `listener` is the serve chain described above. `startCron(overrides)` starts the schedules and hands back a handle whose `stop({ timeoutMs })` waits for the firing in flight for at most `timeoutMs`, resolving `false` if it was still running when that ran out. `dispose(options)` drains the deferred work no invocation carried away and releases the handler's database connection — a host embedding `listener` owes the site this on its own shutdown. `serveWhenMain(main)` runs the site as a process — the `http` server, cron unless `cron: false`, and the shutdown protocol — and does nothing when `main` is false, so importing the entry to embed it serves nothing. The generated entry holds imports and calls only: orchestration lives here, where a compiler and a test can reach it.
