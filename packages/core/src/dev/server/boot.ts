 ...
 * the app (invalid theme registration, plugin setup error, …). Each runtime's
 * entry guards `buildApp` and, in dev, serves this for every request so a boot
 * failure shows the exception, stack, and — when core recognizes the error —
 ...
 * route / database sections). Referenced only under a `process.env.PLUMIX_DEV`
 * gate — inline in Cloudflare's generated entry, in `createNodeSite` for Node —
 * so this module tree-shakes out of production builds, where the boot throw
 * rethrows unchanged.
 */
