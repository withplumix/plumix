...
Writes are unchanged: those tokens are still accepted and still stored as a real boolean, so only a row that bypassed the pipeline can hold one — a legacy `type: "json"` row, an import, or a direct write. Publishing the entry settles such a value to a real boolean.
