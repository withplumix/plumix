...
| `cdn: match` / `cdn: put` | `context/traced-slots.ts` — CDN lookup/store                                              | `cdn.hit` / `cdn.tags`                                         |
...
- **Post-response deferred work** is outside the snapshot: CDN
  `purgeTags`, consumer export latency, and anything else routed through
  `ctx.defer` after the response. The deferred `cdn: put` is the one
