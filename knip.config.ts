...
const config: KnipConfig = {
  ...
  workspaces: {
    ...
    "apps/docs": {
      ...
      // - The other two are how the code-sample check compiles samples: the
      //   ambients a scaffolded app declares, and the tsconfig its options
      //   extend — named in a string, so no import to follow.
      ignoreDependencies: [
        "wrangler",
    ...
    "packages/plumix": {
      entry: [
        ...
        // The dev client error tools' single install entry is loaded by the
        // consumer's generated `.plumix/client-entry.ts` via a dev-gated
        // `import("plumix/core/dev-client")` — a runtime import knip can't see.
        "src/core/dev-client.ts",
    ...
    // Installed with npm against its own lockfile, so the root pnpm install that
    // precedes knip never creates .sandcastle/node_modules and its binaries cannot
    // resolve here. Its own `npm test` and `npm run typecheck` exercise them.
    ".sandcastle": {
      entry: ["lib/*.test.ts"],
      ignoreBinaries: ["tsx", "vitest", "tsc"],
